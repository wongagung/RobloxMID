...
export function createAssetHubRouter(){
  ...
  r.post("/api/playlist-info",(req,res,next)=>{const url=String(req.body?.url||"").trim();try{const parsed=new URL(url);if(!["http:","https:"].includes(parsed.protocol)||!parsed.pathname.toLowerCase().endsWith(".mp3"))return next();const filename=path.basename(parsed.pathname)||"track.mp3";const title=path.basename(filename,path.extname(filename)).replace(/[-_]+/g," ").trim().slice(0,100)||"Track";return res.json({isPlaylist:false,playlistTitle:null,total:1,limited:false,items:[{id:crypto.createHash("sha1").update(url).digest("hex").slice(0,16),title,duration:0,duration_string:"",thumbnail:null,uploader:"",webpage_url:url,playlist_title:null}]});}catch{return next();}});

  r.post("/api/playlist-info",async(req,res,next)=>{
    const url=String(req.body?.url||"").trim();let parsed;try{parsed=new URL(url);}catch{return next();}if(!["http:","https:"].includes(parsed.protocol))return next();
    ...
    const end=maxItems>0?Math.min(maxItems,start+pageSize-1):start+pageSize-1;
    try{
      const cookiesPath=path.join(ROOT,"cookies.txt");const hasCookies=fs.existsSync(cookiesPath)&&fs.statSync(cookiesPath).size>0;
      const args=["--js-runtimes",`node:${process.execPath}`,"--remote-components","ejs:github","--yes-playlist","--flat-playlist","--playlist-start",String(start),"--playlist-end",String(end),"--dump-json","--no-warnings","--retries","3","--socket-timeout","30",...(hasCookies?["--cookies",cookiesPath]:[]),url];
      ...
      const playlistTitle=items.find(x=>x.playlist_title)?.playlist_title||null;const isPlaylist=Boolean(playlistTitle)||items.length>1;const hasNext=isPlaylist&&items.length>=pageSize&&(maxItems===0||end<maxItems);
      res.json({isPlaylist,playlistTitle,total:items.length,page,pageSize,maxItems,hasNext,limited:false,items});
  ...
  // Use the multi-profile YouTube downloader for the primary stream endpoint.
