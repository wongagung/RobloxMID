...
function readHistoryForPlayback(){try{return JSON.parse(fs.readFileSync(path.join(ROOT,"data","history.json"),"utf8"));}catch{return[];}}
...
export function createAssetHubRouter(){
  ...
  // Safe, minimal metadata for WaveDeck normalization. No API keys or history data is exposed.
  r.get("/api/assets/:assetId/playback-meta",(req,res)=>{
    const assetId=String(req.params.assetId||"").trim();
    if(!/^\d{5,20}$/.test(assetId)) return res.status(400).json({ok:false,error:"Asset ID tidak valid."});
    const item=readHistoryForPlayback().find(entry=>String(entry?.roblox?.assetId||"")===assetId);
    const rawSpeed=Number(item?.editorMeta?.speed);
    const sourceSpeed=Number.isFinite(rawSpeed)&&rawSpeed>0?Math.min(8,Math.max(0.05,rawSpeed)):1;
    return res.json({
      ok:true,
      assetId,
      sourceSpeed,
      edited:Boolean(item?.editorMeta),
      name:item?.name||null,
      historyId:item?.id||null,
      editorMeta:item?.editorMeta?{
        speed:sourceSpeed,
        pitch:Number.isFinite(Number(item.editorMeta.pitch))?Number(item.editorMeta.pitch):0,
        trimStart:Number.isFinite(Number(item.editorMeta.trimStart))?Number(item.editorMeta.trimStart):0,
        trimEnd:Number.isFinite(Number(item.editorMeta.trimEnd))?Number(item.editorMeta.trimEnd):null
      }:null
    });
  });
